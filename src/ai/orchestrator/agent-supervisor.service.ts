...
export class AgentSupervisorService {
  ...
  private readonly AGENT_PROFILES: Record<AgentRole, AgentProfile> = {
    CustomerSupportAgent: {
  ...
  /**
   * Evaluates input message and determines the target Agent Profile
   */
  determineAgentProfile(message: string, context: { isOwner?: boolean; hasCart?: boolean } = {}): AgentProfile {
    if (context.isOwner) {
